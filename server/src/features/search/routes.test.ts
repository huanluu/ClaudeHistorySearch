...
function createSearchApp(deps: { repo: SessionRepository; logger?: Logger; indexFn?: SearchRouteDeps['indexFn'] }, withAuth = true): Application {
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    if (!apiKey) {
      return res.status(401).json({ error: 'Unauthorized', message: 'API key required. Provide X-API-Key header.' });
    }
...
describe('Search Routes', () => {
  ...
  beforeAll(() => {
    ...
    app = createSearchApp({ repo: mockRepo }, true);
  });
...
describe('POST /reindex', () => {
  ...
  it('should call indexFn and return result with force=false by default', async () => {
    ...
    const app = createSearchApp({ repo: mockRepo, indexFn }, false);
